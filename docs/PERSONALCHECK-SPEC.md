...
4. **Ficha de personal** — horario semanal, saldo, contador de minutos de tardanza acumulados (solo si es mayor a cero), **contador de horas extra acumuladas** (solo si es mayor a cero — ver nota abajo), timeline de novedades con observaciones, adjunto y **quién la cargó** (cada una con un link **Corregir** — ver punto 5.1); acciones: Cargar novedad / Editar horario / **Eliminar personal**. **AGREGADO 2026-09-22**: quién cargó cada novedad (`movimientos.admin_id`) también se muestra en "Últimas novedades cargadas" del dashboard y como columna en los informes PDF/Excel (y su vista previa) — antes se guardaba pero no se mostraba en ningún lado.

   **AGREGADO 2026-09-24** — contador de horas extra: mismo patrón que el contador de tardanza (histórico acumulado, suma la cantidad de todas las novedades "Hora extra trabajada"), a pedido de Dante ("no tengo como contabilizar el total... de las horas extras trabajadas"). Se muestra formateado en horas y minutos (ej. "10 h 30 min"), no en minutos crudos — a diferencia del contador de tardanza, que sí muestra minutos crudos porque ahí los valores típicos son chicos.
